/***************************************************************************//**
 * @file   adxl345.c
 * @brief  Implementation of ADXL345 Driver for MAX78000 (Maxim SDK).
 * @author DBogdan (dragos.bogdan@analog.com) - Ported for Maxim SDK
********************************************************************************
...
// Note: Using standard malloc/free. Ensure your project's heap is configured.
...
/***************************************************************************//**
 * @brief Reads the value of a register via I2C.
 *
				   ...
				   uint8_t register_address)
{
    ...
    req.i2c = dev->i2c_inst;
    req.addr = dev->i2c_addr;
    req.tx_buf = &register_address;
    req.tx_len = 1;
    req.rx_buf = &register_value;
    req.rx_len = 1;
    req.restart = 0; // Standard I2C Read usually requires Restart, SDK handles it if tx_len > 0 and rx_len > 0
    
    // Perform blocking I2C transaction
    error = MXC_I2C_MasterTransaction(&req);

    if(error != E_NO_ERROR) {
        // Handle error (optional: return a specific error value or log)
        return 0; 
    }
...
/***************************************************************************//**
 * @brief Writes data into a register via I2C.
 *
 * @param dev              - The device structure.
 * @param register_address - Address of the register.
				...
				uint8_t register_value)
{
    ...
    req.i2c = dev->i2c_inst;
    req.addr = dev->i2c_addr;
    req.tx_buf = data_buffer;
    req.tx_len = 2; // Write Reg Addr + Data
    req.rx_buf = NULL;
    req.rx_len = 0;
    req.restart = 0;
		     ...
		     struct adxl345_init_param init_param)
{
	...
	if (!dev)
		return -1; // ENOMEM replacement

    // Initialize device structure with passed parameters
	dev->i2c_inst = init_param.i2c_inst;
    dev->i2c_addr = init_param.i2c_addr;
    dev->dev_type = init_param.dev_type;

    // Initialize the I2C Peripheral
    // Assumes the system clock is configured appropriately.
    // 100kHz or 400kHz are standard for ADXL345.
    status = MXC_I2C_Init(dev->i2c_inst, 1, 0); // 1 = Master Mode
    if (status != E_NO_ERROR) {
        free(dev);
        return status;
    }
    
    // Set Frequency (e.g., 400kHz)
    MXC_I2C_SetFrequency(dev->i2c_inst, 400000);

    // Verify Device ID
	if (adxl345_get_register_value(dev, ADXL345_DEVID) != adxl345_part_id[init_param.dev_type]) {
...
{
    // MAX78000 SDK: MXC_I2C_Shutdown can be called if you want to completely disable the I2C block
    // MXC_I2C_Shutdown(dev->i2c_inst); 
		     ...
		     int16_t* z)
{
    ...
    // Perform burst read of 6 bytes starting at DATAX0
    req.i2c = dev->i2c_inst;
    req.addr = dev->i2c_addr;
    req.tx_buf = &first_reg_address;
    req.tx_len = 1;
    req.rx_buf = read_buffer;
    req.rx_len = 6;
    req.restart = 0;
